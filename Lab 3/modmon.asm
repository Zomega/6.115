...
jumtab:
   ...
   .dw readcmd            ; command 'r' 12 used
   .dw badcmd             ; command 's' 13
   .dw tablecmd           ; command 't' 14 used
   .dw badcmd             ; command 'u' 15
   .dw badcmd             ; command 'v' 16
   .dw writecmd           ; command 'w' 17 used
...
;===============================================================
; command readcmd 'r'
; this routine reads the 4 hex digit address which follows and
...
;===============================================================
; command tablecmd 't'
; writes a table in memory, starting at the specified addr
;===============================================================
tablecmd:
	lcall getbyt           ; get address high byte
	mov   dph, a
	...
	mov   dpl, a
	lcall prthex
	mov r0, #0		; ensure we enter a max of 255 entries
				; TODO: Let user exit out.
	tw_loop:
		lcall crlf
		mov   a, dph
		lcall prthex
